...
typedef struct {
    ...
    XPixel_pack_uio_map maps[ MAX_UIO_MAPS ];
} XPixel_pack_uio_info;

/***************** Variable Definitions **************************************/
static XPixel_pack_uio_info uio_info;
...
static int uio_info_read_name(XPixel_pack_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPixel_pack_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPixel_pack_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPixel_pack_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPixel_pack_Initialize(XPixel_pack *InstancePtr, const char* InstanceName) {
	XPixel_pack_uio_info *InfoPtr = &uio_info;
