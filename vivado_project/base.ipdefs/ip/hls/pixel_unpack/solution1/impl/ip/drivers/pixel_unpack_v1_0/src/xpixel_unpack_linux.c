...
typedef struct {
    ...
    XPixel_unpack_uio_map maps[ MAX_UIO_MAPS ];
} XPixel_unpack_uio_info;

/***************** Variable Definitions **************************************/
static XPixel_unpack_uio_info uio_info;
...
static int uio_info_read_name(XPixel_unpack_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XPixel_unpack_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XPixel_unpack_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XPixel_unpack_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XPixel_unpack_Initialize(XPixel_unpack *InstancePtr, const char* InstanceName) {
	XPixel_unpack_uio_info *InfoPtr = &uio_info;
